...
const setup = async () => {
  ...
  const s3Client = new S3({ region: process.env['AWS_REGION'], endpoint: process.env['AWS_ENDPOINT'] });
  const dbClient = helper.getClient();

  await Promise.all([
    s3Client.createBucket({ Bucket: process.env['BUCKET_NAME_MATERAILS'] as string }),
