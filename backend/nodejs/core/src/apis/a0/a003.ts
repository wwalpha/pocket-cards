...
//   // ユーザ情報検索
//   const userInfo = await DBHelper().get<Tables.TUsers>(Users.get(userId));

//   // ユーザ情報が存在しない
//   if (!userInfo?.Item) return;

//   const { studyQuery, id } = userInfo.Item as Tables.TUsers;
...
//     const { id } = userGroupsInfo.Items[idx] as Tables.TGroups;
...
//     const { lastTime } = groupInfo.Items[0] as Tables.TWords;
...
//           const { id: word, nextTime } = item as Tables.TWords;
...
//   let oldWCU = await getWriteCapacityUnits(client, Environment.TABLE_NAME_WORDS);
...
//     oldWCU = await getWriteCapacityUnits(client, Environment.TABLE_NAME_WORDS);
...
//   // Throughput存在しない
//   if (!result.Tables.ProvisionedThroughput) {
//     return -1;
//   }

//   return result.Tables.ProvisionedThroughput.WriteCapacityUnits;
// };
