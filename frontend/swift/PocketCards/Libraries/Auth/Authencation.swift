    ...
    func signInWithUser(username: String, password: String) {
        ...
        API.request(URLs.SIGN_IN, method: .post, parameters: params)
            ...
            .responseDecodable(of: UserServices.SignIn.Response.self) { response in
                ...
                // save token
                TokenManager.shared.updateIdToken(token: res.idToken)
                TokenManager.shared.updateAccessToken(token: res.accessToken)
                TokenManager.shared.updateRefreshToken(token: res.refreshToken)
